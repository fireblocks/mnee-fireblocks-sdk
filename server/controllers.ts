...
export function getServerInfo(req: Request, res: Response) {
  res.status(200).json({
    ...
    version: "1.0.0",
    cosignerUrl:
      process.env.MNEE_COSIGNER_URL?.replace(/\/v\d+$/, "") || "Not configured",
...
export async function getVaultAddresses(
  ...
  try {
    const vaultAccountId = req.params.vaultAccountId;
    const addresses = await sdk.fireblocksService.getVaultAddressesWithIndexes(
      vaultAccountId
    );
...
export async function transferTokens(
  ...
  try {
    ...
    if (!sourceVaultAccountId) {
      return res
        .status(400)
        .json({ error: "sourceVaultAccountId is required" });
    }
    ...
    const balanceBefore = await sdk.getBalanceForVaultAccount(
    ...
    // Get final balance
    const balanceAfter = await sdk.getBalanceForVaultAccount(
      sourceVaultAccountId
    );
...
export async function getAddressUtxos(
  ...
  try {
    ...
    const utxos = await sdk.cosignerService.fetchUtxos([address], page, limit);
